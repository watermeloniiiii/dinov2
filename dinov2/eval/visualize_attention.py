...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser("Visualize Self-Attention maps", parents=parents)
    parser.add_argument(
        "--arch",
        default="vit_small",
        type=str,
    ...
    parser.add_argument("--image_size", default=480, type=int, nargs="+", help="Resize image.")
    ...
    if args.image_path is None:
        ...
        for band in bands:
            ...
            # if dir.split("/")[-1].startswith("tci"):
            #     img = Image.open(f).convert(mode="RGB")
            # else:
            #     img = Image.open(f).convert(mode="L")
            # img = imread(dir)
            img = np.array(Image.open(args.image_path.replace("tci", band)))
            if len(img.shape) == 2:
                img = img[..., None]
            res.append(img)
        img = np.concatenate([r for r in res], axis=2)
    else:
    ...
    transform = pth_transforms.Compose(
        [
            ...
            pth_transforms.Normalize((0.3584, 0.3111, 0.2654, 0.2578, 0.3299, 0.3653, 0.3547, 0.2965, 0.2266),(0.064, 0.072, 0.0095, 0.048, 0.067, 0.080, 0.085, 0.074, 0.060)),
    ...
    attentions = attentions[0, :, 0, 3:].reshape(nh, -1)
    ...
    #     torchvision.utils.make_grid(img, normalize=True, scale_each=True), os.path.join(args.output_dir, "img.png")
    # )
    for j in range(nh):
